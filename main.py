...
def read_data_nclt(args):
	...
	for idx_i, dataset_i in enumerate(datasets):
		...
		mondict = {'t': t[:i], 'chi': chi[:i], 'u_imu': u_imu[:i], 'u_odo_fog': u_odo_fog[:i], 'y_odo_fog': y_odo_fog[:i], 'y_imu': y_imu[:i], 'name': dataset_i}
...
def set_gp_imu(args, dataset):
	...
	kern_h = gp.kernels.Matern52(input_dim=args.kernel_dim, lengthscale=torch.ones(args.kernel_dim))
	kernel_h = gp.kernels.Warping(kern_h, iwarping_fn=hnet_fn)
	...
	gp_h = gp.models.VariationalSparseGP(u, u.new_ones(9, u.shape[0]), kernel_h, Xu, num_data=dataset.num_data, likelihood=lik_h, mean_function=None, whiten=True, jitter=1e-4)
...
def set_gp_odo_fog(args, dataset):
	...
	Xu = u[torch.arange(0, u.shape[0], step=int(u.shape[0]/args.num_inducing_point)).long()]
	print("main_kaist, Xu size:", Xu.shape)
	print("main_kaist, u size:", u.shape)
	...
	kern_f = gp.kernels.Matern52(input_dim=args.kernel_dim, lengthscale=torch.ones(args.kernel_dim))
	kernel_f = gp.kernels.Warping(kern_f, iwarping_fn=fnet_fn)
	...
	gp_f = gp.models.VariationalSparseGP(u, u.new_ones(6, u.shape[0]), kernel_f, Xu, num_data=dataset.num_data, likelihood=lik_f, mean_function=None, whiten=True, jitter=1e-4)
...
def post_tests(args, dataset, filter_original):
	...
	for i in range(len(dataset.datasets)):
		...
		p = x_corrected[:, :2]
		p1 = x_original[:, :2]
		plt.plot(chi[:, 0, 3].numpy(), chi[:, 1, 3].numpy(), color="k", label='Ground Truth')
		plt.plot(p1[:, 0].numpy(), p1[:, 1].numpy(), color="b", ls=':', label='VIWO')
		plt.plot(p[:, 0].numpy(), p[:, 1].numpy(), color="r", ls='--', label='VIWO with corrected WO') #niraj from filter.py
		plt.plot(p[0, 0].numpy(), p[0, 1].numpy(), marker="s", markersize=10, markeredgecolor="red", markerfacecolor="green", label='Starting point')
		plt.title("Trajectory for " + dataset_name)
		plt.legend(loc="upper left")
		plt.grid(True)
		plt.savefig("results/kaist/" + dataset_name + "plot.png")
		plt.close()

		fig, (ax1, ax2) = plt.subplots(1, 2)

		label = ["corrected", "un-corrected"]
		error_mate_tra = [error_corrected['mate translation'], error_original['mate translation']]
		error_mate_rot = [error_corrected['mate rotation']*180/np.pi, error_original['mate rotation']*180/np.pi]
		positions = range(len(error_mate_tra))
		ax1.bar(positions, error_mate_tra, color=('g', 'r'))
		ax1.set_xticks(positions, label)
		ax1.set_ylabel('mean ATE (m)')

		ax2.bar(positions, error_mate_rot, color=('g', 'r'))
		ax2.set_xticks(positions, label)
		ax2.set_ylabel('mean RTE (deg)')
		fig.tight_layout()
		fig.savefig("results/kaist/" + dataset_name + "M_ATE_RTE_barplot.png")
...
def launch(args):
	if args.nclt:
		...
		args.cross_validation_sequences = ['urban14', 'urban16']
		args.test_sequences = ['urban17', 'urban39'] #['urban15', 'urban16']
...
if __name__ == '__main__':
	...
	# paths
	parser.add_argument('--path_data_base', type=str, default="C:/Users/niraj/Desktop/Dev/Datasets/data/kaist/")

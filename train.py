...
class GpOdoFog(Parameterized):
	...
	def correct(self, x, u_odo, u_fog, compute_G=False, full_cov=False):
		...
		#x[3:6] = SO3.from_matrix(Rot.mm(dRot_cor), normalize=True).to_rpy()
		return x, G_cor
...
def train_gp(args, dataset, gp_class):
	...
	if gp_class.name == 'GpOdoFog':
		...
		#kernel = gp.kernels.Matern52(input_dim=args.kernel_dim, lengthscale=torch.ones(args.kernel_dim)).Warping(iwarping_fn= fnet_fn)
		kern = gp.kernels.Matern52(input_dim=args.kernel_dim, lengthscale=torch.ones(args.kernel_dim))
		kernel = gp.kernels.Warping(kern,iwarping_fn=fnet_fn)
		Xu = u[torch.arange(0, u.shape[0], step=int(u.shape[0]/args.num_inducing_point)).long()]
		print("train.py - train,Xu size:", Xu.shape)
		print("train.py - train, u size:", u.shape)
		print("train.py - train, y size:", y.shape)
		#gp_model = gp.models.VariationalSparseGP(u, torch.zeros(6, u.shape[0]), kernel, Xu, num_data=dataset.num_data, likelihood=lik,
		#										 mean_function=None,name=gp_class.name, whiten=True, jitter=1e-3)
		gp_model = gp.models.VariationalSparseGP(u, torch.zeros(6, u.shape[0]), kernel, Xu, num_data=dataset.num_data,
												 likelihood=lik, mean_function=None, whiten=True, jitter=1e-3)
	else:
		...
		kern = gp.kernels.Matern52(input_dim=args.kernel_dim, lengthscale=torch.ones(args.kernel_dim))
		kernel = gp.kernels.Warping(kern, iwarping_fn=hnet_fn)
		Xu = u[torch.arange(0, u.shape[0], step=int(u.shape[0]/args.num_inducing_point)).long()]
		#gp_model = gp.models.VariationalSparseGP(u, torch.zeros(9, u.shape[0]), kernel, Xu, num_data=dataset.num_data, likelihood=lik, mean_function=None,name=gp_class.name, whiten=True, jitter=1e-4)
		gp_model = gp.models.VariationalSparseGP(u, torch.zeros(9, u.shape[0]), kernel, Xu, num_data=dataset.num_data, likelihood=lik, mean_function=None, whiten=True, jitter=1e-4)
	...
	plt.plot(train_losses, label="train loss vs epochs")
	plt.xlabel("epochs")
	plt.ylabel("Loss")
	plt.legend()
	plt.grid(True)
	plt.savefig("results/kaist/" + "training loss" + "plot.png")
	plt.close()

	plt.figure()
	plt.title("Training time")
	#plt.plot(val_losses, label="val")
	plt.plot(epoch_time, label="training time vs epochs")
	plt.xlabel("epochs")
	plt.ylabel("time (sec)")
	plt.legend()
	plt.grid(True)
	plt.savefig("results/kaist/" + "training time" + "plot.png")
	plt.close()